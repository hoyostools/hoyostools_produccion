...
class SaleOrder(models.Model):
    ...
    def add_order_message(self, action="shipping", company_id=None, force=False, config=None):
        ...
        logger.debug(f"PREPARING MESSAGE {action}")
    ...
    def get_invoice_xml(self, order_id):
        ...
        for invoice in order.invoice_ids:
            ...
            if order.yuju_invoice_doctype and order.yuju_invoice_doctype in ["invoice", "ticket"]:
                # Si el pedido tiene un tipo de documento definido, se usa ese
                order.message_post(body=f"Se usa tipo de documento de la orden {order.yuju_invoice_doctype}")
                doc_type = order.yuju_invoice_doctype
