                    ...
                    for row in myreader:
                        try:
                            ...
                            if row_field_error_dic:
                                if '' not in row_field_error_dic:
                                    res = self.show_success_msg(
                                        0,0, row_field_error_dic)
                                    return res
                            ...
                            if row[3].strip() not in ("", None, False):
                                journal_id = self.env['account.journal'].sudo().search(
                                    [('name', '=', row[3].strip()),('company_id','=',self.company_id.id)], limit=1)
                                if journal_id:
                                    vals.update({
                    ...
                    for row in range(sheet.nrows):
                        try:
                            ...
                            if sheet.cell(row, 3).value.strip() not in ("", None, False):
                                journal_id = self.env['account.journal'].sudo().search(
                                    [('name', '=', sheet.cell(row, 3).value.strip()),('company_id','=',self.company_id.id)], limit=1)
                                if journal_id:
                                    vals.update({
                                        'journal_id': journal_id.id
                                    })
