...
class PosConfig(models.Model):
    ...
    def get_stock_transfer_list(self):
        ...
        main['picking_type'] = self.env['stock.picking.type'].search_read(
            ...
            ['display_name', 'code', 'load_default_pos'])
        main['location'] = self.env['stock.location'].search_read([], [
            'display_name', 'load_default_origin_pos', 'load_default_dest_pos'])
        main['wh_stock'] = []
        for record in self.env['stock.warehouse'].search([('company_id', '=', self.env.user.company_id.id)]):
            main['wh_stock'].append(record.lot_stock_id.id)
        return main
