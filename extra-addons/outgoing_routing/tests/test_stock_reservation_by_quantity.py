...
class TestStockRouting(TransactionCase):
    ...
    def test_stock_reservation_by_quantity_case2(self):
        quants = self.stock_quant_model.with_context(
            reservation_strategy=self.env.user.company_id.stock_reservation_strategy,
            reservation_quantity=12,
        )._get_reserve_quantity(self.product_Z, self.stock_A, 12)
        self._update_quants_reservation(quants)
