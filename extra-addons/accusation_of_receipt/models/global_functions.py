...
def get_cufe_cude_de(
    ...
    for name, value in variables.items():
        # Si alguno es booleano, lo convertimos en cadena vacía (o lanza error si quieres forzar)
        if isinstance(value, bool):
            raise ValidationError(f"⚠️ El campo {name} tiene un valor booleano ({value}). Se esperaba texto.")
        # Si es None, lo convertimos a cadena vacía
        if value is None:
            variables[name] = ""
        else:
            variables[name] = str(value)

    # --- CONSTRUCCIÓN DEL CUDE ---
    uncoded_value = (
        variables["Num_DE"] + " + " +
        variables["Fec_Emi"] + " + " +
        variables["Hor_Emi"] + " + " +
        variables["NitFE"] + " + " +
        variables["DocAdq"] + " + " +
        variables["ResponseCode"] + " + " +
        variables["ID_D"] + " + " +
        variables["DocumentTypeCode"] + " + " +
        variables["SoftwarePIN"]
    )

    unicode_val = unidecode(
        variables["Num_DE"] +
        variables["Fec_Emi"] +
        variables["Hor_Emi"] +
        variables["NitFE"] +
    ...
    CUFE_CUDE = hashlib.sha384(unicode_val)
