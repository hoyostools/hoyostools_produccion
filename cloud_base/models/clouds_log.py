...
class clouds_log(models.Model):
    ...
    @api.model
    def action_prepare_logs_html(self, log_args={}, search_name=False, selected_clients=[], log_levels=[], period=[]):
        """
        ...
        """
        log_ids = self._search_logs_by_params(log_args, search_name, selected_clients, log_levels, period, True)
    ...
    @api.model
    def _prepare_txt_logs(self, search_name=False, selected_clients=[], log_levels=[], period=[]):
        """
         ...
         * text
        """
        log_ids = self._search_logs_by_params({}, search_name, selected_clients, log_levels, period, False)
        log_text = ""
        for log in log_ids:
    ...
    def _search_logs_by_params(
        self, log_args={}, search_name=False, selected_clients=[], log_levels=[], period=[], search_limit=False
    ):
        """
        ...
        if period:
            if period[0]:
                domain.append(("name", ">=", period[0]))
            if period[1]:
                domain.append(("name", "<=", period[1]))
        logs_limit = LOGS_LIMIT + 1
        if not search_limit:
            logs_limit = self.env["ir.config_parameter"].sudo().get_param("cloud_base.max_log_export", "50000")
            try:
                logs_limit = int(logs_limit)
            except:
                logs_limit = 50000
            logs_limit = logs_limit > 0 and logs_limit or 50000
        log_ids = self.search(domain, limit=logs_limit)
        return log_ids
