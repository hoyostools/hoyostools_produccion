...
class AccountInvoiceDianDocument(models.Model):
    ...
    qr_code_dian_img = fields.Text(string="QR Base64", compute="_compute_qr_code_dian_img", store=False)
    ...
    @api.depends('cufe_cude')
    def _compute_qr_code_dian_img(self):
        for record in self:
            if record.cufe_cude:
                record.qr_code_dian_img = self._generate_qr_base64(
                    f"https://catalogo-vpfe.dian.gov.co/document/searchqr?documentkey={record.cufe_cude}"
                )
            else:
                record.qr_code_dian_img = False
    ...
    def _generate_qr_code(self):
        if self.invoice_id.move_type in ('out_invoice', 'out_refund'):
            ...
            if self.invoice_id.currency_id.name != 'COP':
                amount_untaxed = self.invoice_id.amount_untaxed * self.invoice_id.trm
            else:
    ...
    def _get_GetStatus_values(self):
        for record in self:
            xml_soap_values = global_functions.get_xml_soap_values(
                    record.company_id.certificate_file,
                    record.company_id.certificate_password)
            if xml_soap_values:
                xml_soap_values['trackId'] = record.cufe_cude
    ...
    def _get_pdf_file(self):
        ...
        if template:
            pdf = template._render_qweb_pdf(template,self.invoice_id.id)
        else:
            pdf = self.env.ref('account.account_invoices')._render_qweb_pdf('account.account_invoices',self.invoice_id.id)
        pdf = pdf[0]
    ...
    def send_failure_email(self):
        ...
        subject = _(
            'ALERTA! La Factura %s no fue enviada a la DIAN.') % self.invoice_id.name
    ...
    def _get_xml_suppplier_values(self, ClTec):
        ...
        CurrencyID = self.invoice_id.currency_id.name

        if self.invoice_id.currency_id.name != 'COP':
            amount_untaxed = self.invoice_id.amount_untaxed * self.invoice_id.trm
            DocumentCurrencyCode = self.company_id.currency_id.name
        else:
            amount_untaxed = self.invoice_id.amount_untaxed
        ...
        software_security_code = global_functions.get_software_security_code(
            ...
            ID)
        partition_key = 'co|' + \
            str(IssueDate).split('-')[2] + '|' + cufe_cude['CUFE/CUDE'][:2]
        emission_date = str(IssueDate).replace('-', '')
        QRCodeURL = QRCodeURL.format(
            cufe_cude['CUFE/CUDE'], partition_key, emission_date)
    ...
    def _get_accuse_recibo_values(self):
        ...
        xml_values['InvoiceLines'] = self.invoice_id._get_invoice_lines18()
    ...
    def _get_attachment_values(self):
        ...
        xml_values['InvoiceTypeCode'] = self.invoice_id.invoice_type_code
        xml_values['InvoiceLines'] = self.invoice_id._get_invoice_lines18()
        xml_values['ApplicationResponse'] = b64decode(
            self.ar_xml_file).decode("utf-8", "ignore")
        xml_values['xml_file'] = b64decode(
            self.xml_file).decode("utf-8", "ignore")
    ...
    def _get_credit_note_values(self):
        ...
        if billing_reference:
            ...
            self.invoice_id.set_invoice_period_dates()

            start_date = self.invoice_id.period_start_date
            end_date = self.invoice_id.period_end_date

            xml_values.update({
                'InvoicePeriod': {
                    'StartDate': start_date.strftime('%Y-%m-%d') if start_date else '',
                    'StartTime': '00:00:00' if start_date else '',
                    'EndDate': end_date.strftime('%Y-%m-%d') if end_date else '',
                    'EndTime': '23:59:59' if end_date else '',
                }
            })
        else:
            xml_values['CustomizationID'] = '20'
        ...
        xml_values['CreditNoteLines'] = self.invoice_id._get_invoice_lines18()
        return xml_values
    ...
    def _get_debit_note_values(self):
        ...
        xml_values['DiscrepancyResponseCode'] = self.invoice_id.discrepancy_response_code_id.code
        xml_values['DiscrepancyDescription'] = self.invoice_id.discrepancy_response_code_id.name
        xml_values['DebitNoteLines'] = self.invoice_id._get_invoice_lines18()
        return xml_values

    def accuse_recibo(self):
        accepted_xml_without_signature = global_functions.get_template_xml(
            self._get_accuse_recibo_values(), 'AcuseRecibo')
        accepted_xml_with_signature = global_functions.get_xml_with_signature(
            accepted_xml_without_signature, self.company_id.signature_policy_url, self.company_id.signature_policy_description, self.company_id.certificate_file, self.company_id.certificate_password)
        self.write({'exp_accepted_file': b64encode(self._get_acp_zipped_file(
            accepted_xml_with_signature)).decode("utf-8", "ignore")})
    ...
    def bs_acceptation(self):
        accepted_xml_without_signature = global_functions.get_template_xml(
            self._get_accepted_values(), 'ReciboBS')
        accepted_xml_with_signature = global_functions.get_xml_with_signature(
            accepted_xml_without_signature, self.company_id.signature_policy_url, self.company_id.signature_policy_description, self.company_id.certificate_file, self.company_id.certificate_password)
    ...
    def express_acceptation(self):
        accepted_xml_without_signature = global_functions.get_template_xml(
            self._get_accepted_values(), 'AceptacionExpresa')
        accepted_xml_with_signature = global_functions.get_xml_with_signature(
            accepted_xml_without_signature, self.company_id.signature_policy_url, self.company_id.signature_policy_description, self.company_id.certificate_file, self.company_id.certificate_password)
        self.write({'exp_accepted_file': b64encode(self._get_acp_zipped_file(
            accepted_xml_with_signature)).decode("utf-8", "ignore")})
    ...
    def refund_acceptation(self):
        accepted_xml_without_signature = global_functions.get_template_xml(
            self._get_accepted_values(), 'ReclamoEI')
        accepted_xml_with_signature = global_functions.get_xml_with_signature(
            accepted_xml_without_signature, self.company_id.signature_policy_url, self.company_id.signature_policy_description, self.company_id.certificate_file, self.company_id.certificate_password)
        self.write({'exp_accepted_file': b64encode(self._get_acp_zipped_file(
            accepted_xml_with_signature)).decode("utf-8", "ignore")})
    ...
    def _get_xml_file(self):
        ...
        if self.invoice_id.move_type == "out_invoice" and not self.invoice_id.refund_type:
            ...
            xml_without_signature = global_functions.get_template_xml(
                self._get_support_values(), 'SupportDocument')
        elif self.invoice_id.move_type == "in_refund":
            xml_without_signature = global_functions.get_template_xml(
                self._get_support_values(), 'SupportDocumentCredit')
    ...
    def action_set_files(self):
        ...
        self.write({'xml_file': b64encode(
            self._get_xml_file()).decode("utf-8", "ignore")})
        self.write({'zipped_file': b64encode(
            self._get_zipped_file()).decode("utf-8", "ignore")})
    ...
    def _get_SendTestSetAsync_values(self):
        ...
        xml_soap_values['contentFile'] = self.zipped_file.decode(
            "utf-8", "ignore")
    ...
    def _get_AcceptedSendTestSetAsync_values(self, accepted_file):
        ...
        xml_soap_values['fileName'] = self.zipped_filename.replace('.zip', '')
        xml_soap_values['contentFile'] = accepted_file.decode(
            "utf-8", "ignore")
        xml_soap_values['testSetId'] = self.company_id.test_set_id
        return xml_soap_values
    ...
    def action_sent_accepted_file(self, accepted_file):
        ...
        if self.company_id.profile_execution_id == '1':
            wsdl = DIAN['wsdl']
            SendBillAsync_values = self._get_AcceptedSendBillAsync_values(
                accepted_file)
            SendBillAsync_values['To'] = wsdl.replace('?wsdl', '')
            xml_soap_with_signature = global_functions.get_xml_soap_with_signature(
                global_functions.get_template_xml(
                    SendBillAsync_values, 'SendBillSync'),
                SendBillAsync_values['Id'],
                self.company_id.certificate_file,
                self.company_id.certificate_password)
        else:
            SendTestSetAsync_values = self._get_AcceptedSendTestSetAsync_values(
                accepted_file)
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                raise ValidationError(
                    msg1 % (response.status_code, response.reason))
    ...
    def action_GetStatusZip(self):
        ...
        xml_soap_with_signature = global_functions.get_xml_soap_with_signature(
            global_functions.get_template_xml(
                GetStatusZip_values, 'GetStatusZip'),
    ...
    def GetStatusZip(self):
        ...
        if response.status_code == 200:
            ...
            # Validación para extraer y asignar CUFE correcto
            for element in root.iter("{%s}string" % c):
                try:
                    decoded_file = b64decode(element.text)
                    xml_decoded = decoded_file.decode('utf-8')
                    
                    # Verificar si el XML contiene el nombre o el UUID de esta factura
                    invoice_number = self.invoice_id.name or ''
                    invoice_uuid = self.invoice_id.uuid or ''
                    if invoice_number in xml_decoded or invoice_uuid in xml_decoded:
                        # Buscar el CUFE en el XML
                        cufe_match = re.search(r'<cbc:UUID.*?>(.*?)</cbc:UUID>', xml_decoded)
                        if cufe_match:
                            cufe = cufe_match.group(1)
                            self.cufe_cude = cufe
                            _logger.info(f"CUFE asignado a {invoice_number}: {cufe}")
                        break  # No seguir procesando otros XML
                except Exception as e:
                    _logger.warning(f"Error procesando archivo DIAN: {e}")
    ...
    def change_cufe(self):
        if 'procesado anteriormente.' in self.get_status_zip_response and not self.origin_cufe_cude:
            ...
            new_cufe = self.get_status_zip_response.replace(
                "- Regla: 90, Rechazo: Documento con CUFE '", '').replace("' procesado anteriormente.", '')
